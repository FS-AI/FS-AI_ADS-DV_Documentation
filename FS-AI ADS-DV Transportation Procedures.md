...
2. [Transport Inventory](#2-transport-inventory)
3. [Preparing the ADS-DV for Movement](#3-preparing-the-ads-dv-for-movement)
4. [ADS-DV Movement Procedure](#4-ads-dv-movement-procedure)
5. [Protection and Securing During Transport](#5-protection-and-securing-during-transport)
...
The following equipment and accessories must accompany the ADS-DV during transportation. Without the required control items, the Brake Discharge procedure cannot be completed and the ADS-DV cannot be pushed on its wheels when required for controlled handling.
...
Before any movement of the ADS-DV for transport, complete the Preparation for Storage procedure in [FS-AI ADS-DV User Manual, Section 7](./FS-AI%20ADS-DV%20User%20Manual.md#7-preparation-for-storage).

For transport purposes, this means:

- Connecting the MPP Dongle, enabling the RES, completing the E-Stop Loop, and turning "On" the LV Master Switch.
- Verifying the Traction Battery voltage is above 50.0V and charging it if required before storage.
- Using the "Brake Discharge" switch on the Touch Screen "Service" page until the Brake Pressures are depleted and the wheels can be turned manually.
- Turning "Off" the LV Master Switch and the RES, and storing the RES, "Red Keys", and MPP Dongle safely.

This Brake Discharge step is required before the ADS-DV can be pushed on its wheels for controlled handling, but the ADS-DV should then be returned to the fully powered-down storage state before transport movement begins.
...
Moving / manoeuvring the ADS-DV requires a minimum of three personnel. The following roles are defined:

#### Safety Supervisor

The Safety Supervisor must not perform any other task. They must observe all movement of the ADS-DV, ensure the intended route is clear, and instruct the Vehicle Handlers to stop immediately if any unsafe condition arises.
...
Use the on-wheel movement method only where necessary, including:

- manoeuvring the ADS-DV up or down kerbs or similar level changes using the supplied plastic ramps
- positioning the ADS-DV inside the transport vehicle or trailer ready for securing
...
## 5. Protection and Securing During Transport

### 5.1. Protection During Transport

The ADS-DV and transport inventory must only be transported inside a covered and locked car carrier, or by a covered and locked trailer. Transportation using other vehicle types, including Luton vans with tail lifts, is not permitted.
...
### 6.1. IMechE Formula Student Office

- [fs@imeche.org](mailto:fs@imeche.org)

### 6.2. IMechE Formula Student AI Technical Support

- [ian.murphy@member.imeche.org](mailto:ian.murphy@member.imeche.org)
