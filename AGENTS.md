...
- `FS-AI ADS-DV Operational Safety.md` is a substantial draft safety document covering storage/access, handling/lifting, and static/dynamic operating controls.
...
- `FS-AI ADS-DV Operational Safety.md` is materially populated, but it still includes placeholder publication metadata and should be kept aligned with Transportation Procedures where transport-related handling rules overlap.
...
- `FS-AI ADS-DV Transportation Procedures.md` is now a substantially rewritten numbered draft aligned to the current user manual. It now requires completion of User Manual Section 7 before transport movement, uses trolley movement as the default handling method, allows on-wheel movement only where necessary, and states that the ADS-DV is transported on its own wheels rather than on the trolleys. It still contains photo placeholders and publication metadata placeholders.
