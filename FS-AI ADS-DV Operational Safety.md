...
5. [Static Operation](#5-static-operation)
6. [Dynamic Operation](#6-dynamic-operation)
7. [Wet Weather Operation](#7-wet-weather-operation)
8. [Airports and RF Interference](#8-airports-and-rf-interference)
...
- The ADS-DV may only be accessed by authorised personnel who have been briefed on the ADS-DV and have read this document, the RAMS and the User Manual.
...
**The ADS-DV weighs 300kg, which means each wheel carries 75kg. Thus, a single person at each wheel is insufficient to lift the ADS-DV safely and must not be attempted.**
...
- The stands used must conform to the following requirements:
  ...
  - At least 800mm x 400mm spread of support under the chassis to avoid rocking or tipping.
...
- Personnel should wear suitable weather protection. In strong sun, sun protection such as hats, sunscreen, and appropriate clothing is recommended.
- Drinking water should be readily available for all personnel. Food should also be available if operations are prolonged.
- If the Dynamic Operating Area is remote, for example an airfield, welfare arrangements should be planned in advance so that personnel have access to shelter, toilets, drinking water, food, and other essentials for the duration of the operation.
- A plan should be in place to deal with a medical emergency or injury, including how assistance will be summoned and how the location will be communicated clearly to emergency services if required.

> **Note:** The ADS-DV must not be left out in the rain to get wet. A waterproof car cover is provided to protect the ADS-DV in the event of rain, should there be no other cover available.

### 6.2. Dynamic Operation Requirements
...
If the designated Dynamic Operating Area requires it, the IMechE will specify hard-coded Operational Limits to be placed on ADS-DV performance. For example, maximum speed, maximum torque (acceleration), or maximum steering angle for narrow areas.
...
## 7. Wet Weather Operation

The ADS-DV is not fully waterproof, especially the InCarPC. Therefore, ADS-DV operation in wet weather is only permissible according to the following conditions:
...
## 8. Airports and RF Interference

The ADS-DV Remote Emergency Stop (RES) operates on a frequency range of 430MHz to 440MHz. It has been observed previously that operation of the RES may be affected by RF systems present at airports.

If an airport is to be designated as a Dynamic Operating Area, the airport should be informed that the RES radiates in this frequency band and must indicate approval.
